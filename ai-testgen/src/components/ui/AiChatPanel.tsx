...
export function AiChatPanel({ testCases, testCaseId, userId, onTestCasesUpdated }: AiChatPanelProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-black/20 border-l border-white/[0.04]">
      {/* Header */}
      <div className="px-5 py-4 border-b border-white/[0.06] bg-black/30 shrink-0">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[10px] text-muted-foreground/50 uppercase tracking-[0.15em] font-bold">
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4 scroll-smooth">
        ...
        {messages.length === 0 && !loadingHistory && (
          <div className="flex flex-col items-center justify-center h-full text-center px-4 space-y-6">
            <div className="w-14 h-14 rounded-2xl bg-violet-500/10 flex items-center justify-center border border-violet-500/15">
            ...
            <div className="flex flex-wrap justify-center gap-2 max-w-[320px]">
              {QUICK_ACTIONS.map((action) => (
                <button
                  ...
                  disabled={loading}
                  className="px-3 py-1.5 rounded-lg bg-white/[0.03] hover:bg-white/[0.06] text-foreground/70 text-[11px] font-medium transition-all border border-white/[0.08] hover:border-violet-500/20 disabled:opacity-50"
                >
                  {action.label}
                </button>
              ))}
          ...
          >
            <div className={`max-w-[85%] flex gap-2 ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
              ...
              <div className={`w-7 h-7 rounded-lg flex items-center justify-center shrink-0 mt-1 ${
                msg.role === 'user' ? 'bg-violet-500/15 border border-violet-500/20' : 'bg-white/[0.04] border border-white/[0.06]'
              }`}>
                {msg.role === 'user' 
              ...
              <div className={`px-4 py-3 rounded-2xl text-sm leading-relaxed ${
                msg.role === 'user'
                  ? 'bg-violet-500/10 text-foreground border border-violet-500/15 rounded-tr-md'
                  : 'bg-white/[0.03] text-foreground/80 border border-white/[0.06] rounded-tl-md'
              }`}>
        ...
        {loading && (
          <div className="flex justify-start animate-in fade-in duration-300">
            <div className="flex gap-2">
              <div className="w-7 h-7 rounded-lg bg-white/[0.04] border border-white/[0.06] flex items-center justify-center shrink-0 mt-1">
                <Sparkles className="w-3.5 h-3.5 text-muted-foreground/60 animate-pulse" />
              </div>
              <div className="px-4 py-3 rounded-2xl rounded-tl-md bg-white/[0.03] border border-white/[0.06]">
                <div className="flex items-center gap-2">
                  <Loader2 className="w-3.5 h-3.5 animate-spin text-violet-400" />
                  <span className="text-sm text-muted-foreground/60">AI is thinking...</span>
                </div>
              </div>
            </div>
          </div>
      ...
      {messages.length > 0 && !loading && (
        <div className="px-4 py-2 border-t border-white/[0.04] flex gap-1.5 overflow-x-auto shrink-0 scrollbar-none">
          {QUICK_ACTIONS.map((action) => (
            <button
              key={action.label}
              onClick={() => handleSend(action.instruction)}
              disabled={loading}
              className="px-2.5 py-1 rounded-lg bg-white/[0.02] hover:bg-white/[0.05] text-[10px] font-medium transition-all border border-white/[0.06] hover:border-violet-500/20 whitespace-nowrap shrink-0 disabled:opacity-50 text-foreground/60"
            >
              {action.label}
            </button>
          ))}
      ...
      {/* Input Area */}
      <div className="p-4 border-t border-white/[0.06] bg-black/30 shrink-0">
        <div className="flex gap-2 items-end">
          <textarea
            ...
            rows={1}
            disabled={loading}
            className="flex-1 px-4 py-3 bg-black/40 border border-white/[0.08] focus:border-violet-500/40 focus:ring-1 focus:ring-violet-500/20 rounded-xl outline-none transition-all text-sm text-foreground placeholder:text-muted-foreground/40 resize-none min-h-[44px] max-h-[120px] disabled:opacity-50"
          ...
          <button
            ...
            className="w-11 h-11 rounded-xl bg-gradient-to-br from-violet-600 to-indigo-600 text-white flex items-center justify-center hover:from-violet-500 hover:to-indigo-500 transition-all disabled:opacity-50 shrink-0 shadow-lg shadow-violet-500/15"
          >
